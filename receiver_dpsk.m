function varargout = receiver_dpsk(ich,x)

%RECEIVER_DPSK Complete DPSK receiver. (POST fiber+OBPF+MZ+LPF).
%   VARARGOUT=RECEIVER_DPSK(ICH,X) returns the received current of a DPSK 
%   transmission using the following receiver:
%
%          __                    Mach Zehnder     __|__
%         /  \                    ---------        / \
%        |    |     -------     /          \   /   ---     -------
%  sig    \  /     |       |   /            \ /     |     |       | IRIC   
%     -------------| OBPF  |---              -      |-----|  LPF  |-------
...
%   complete case it is VARARGOUT=[IRIC,SN], where Iric is a vector
%   containing the received current of channel ICH, while SN is a vector 
...
%           interferometer for DPSK. The default delay is 1 and it must be 
%           comprised in the interval 0 < mzdel <=1. Setting mzdel to a 
%           value smaller than 1 implements the Partial DPSK (P-DPSK) [1]. 
%
...
%
%   See also RECEIVER_OOK, RECEIVER_DQPSK
%
%   [1] B. Mikkelsen, C. Rasmussen, P. Mamyshev and F. Liu, "Partial DPSK 
%       with excellent filter tolerance and OSNR sensitivity," Electronics
...
if isy
    ...
    x.sigx = ifft(x.sigx);
    x.sigy = ifft(x.sigy);    
    Iric = real(x.sigx.*conj(x.sigx(ndel)))+real(x.sigy.*conj(x.sigy(ndel)));       
else
    if nargout == 2    
        varargout(2) = {x};
    end
    x.sigx = ifft(x.sigx);
    Iric = real(x.sigx.*conj(x.sigx(ndel)));     % 2/3: photodiode    
end
                                                % 3/3: lowpass filter
