function varargout = receiver_ook(ich,x)

%RECEIVER_OOK Complete OOK receiver (POST fiber+OBPF+photodiode+LPF).
%   VARARGOUT=RECEIVER_OOK(ICH,X) returns the received current of an OOK 
...
if isy
    ...
    x.sigy = x.sigy .* Hf;
    Iric = abs(ifft(x.sigx)).^2 + abs(ifft(x.sigy)).^2;  % 2/3: photodiode    
else
    Iric = abs(ifft(x.sigx)).^2;
end
                                                % 3/3: lowpass filter
